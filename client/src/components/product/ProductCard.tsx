   ...
   <li className='bg-white rounded-lg shadow-lg hover:shadow-2xl transition-all duration-300 hover:scale-[1.02] flex flex-col h-full overflow-hidden'>
      <div className='p-4 flex flex-col flex-grow'>
         <div className='relative w-full aspect-square border-2 border-border rounded-md overflow-hidden mb-3'>
          ...
          <Link className="absolute inset-0 z-10" title="More Info" to={`/product/${product.slug}`}>
            <span className="sr-only">View {product.name}</span>
          </Link>
        </div>

        <div className="mb-2 text-center">
          <Link to={`/product/${product.slug}`} className="text-base font-semibold text-text-primary hover:text-[var(--color-link-hover)] transition-colors">
            {product.name}
          </Link>
        </div>
        ...
        <div className="flex flex-wrap gap-1 mb-3 justify-center">
          {product.isNew && (
            <span className={badgeStyles.new}>NEW</span>
          )}
          {product.isExclusive && (
        ...
        <div className="space-y-1 mb-3">
          <div className="flex justify-between items-center gap-2">
            ...
            <span className="text-sm font-semibold text-[var(--color-info)] text-right">{formatPrice(product.price, product.priceIcon)}</span>
          </div>
          <div className="flex justify-between items-center gap-2">
