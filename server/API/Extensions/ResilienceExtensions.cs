...
public static class ResilienceExtensions
{
    public static IServiceCollection AddPollyPolicies(this IServiceCollection services)
    {
        services.AddHttpClient("PollyClient")
            .AddPolicyHandler((serviceProvider, request) => BuildPollyPolicy(serviceProvider));
        
        return services;
    }

    // NOTE: Allow overriding delays to speed up unit tests, leave as default for production
    public static IAsyncPolicy<HttpResponseMessage> BuildPollyPolicy(IServiceProvider serviceProvider, 
        TimeSpan? retryDelay = null, TimeSpan? timeout = null)
    {
        ...
        var retryPolicy = Policy<HttpResponseMessage>
            ...
            .WaitAndRetryAsync(
                retryCount: 3,
                sleepDurationProvider: retryAttempt => retryDelay ?? TimeSpan.FromSeconds(Math.Pow(2, retryAttempt)),
        ...
        var timeoutPolicy = Policy.TimeoutAsync<HttpResponseMessage>(timeout ?? TimeSpan.FromSeconds(10));
