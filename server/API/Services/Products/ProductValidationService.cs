...
{
    Task<Result<bool>> ValidateProductAsync(ProductFormDto dto, Product? originalProduct = null, CancellationToken ct = default);
    Task<Result<bool>> ValidateSubcategoryIdsAsync(List<int> subcategoryIds, CancellationToken ct = default);
}
...
public class ProductValidationService : IProductValidationService
{
    ...
    public async Task<Result<bool>> ValidateProductAsync(ProductFormDto dto, Product? originalProduct = null, CancellationToken ct = default)
    {
        if (await _queryExecutor.ExistsByFieldAsync<Product>("name", dto.Name, ct) && (originalProduct == null || originalProduct.Name != dto.Name))
            return Result<bool>.Failure(ErrorMessages.Product.NameExists(dto.Name));
        if (await _queryExecutor.ExistsByFieldAsync<Product>("slug", dto.Slug, ct) && (originalProduct == null || originalProduct.Slug != dto.Slug))
            return Result<bool>.Failure(ErrorMessages.Product.SlugExists(dto.Slug));
        if (dto.SubcategoryIds.Count != 0)
        {
            var subcategoryValidationResult = await ValidateSubcategoryIdsAsync(dto.SubcategoryIds, ct);
    ...
    public async Task<Result<bool>> ValidateSubcategoryIdsAsync(List<int> subcategoriesIds, CancellationToken ct = default)
    {
        ...
        var existingSubcategories =
            (await _queryExecutor.GetWhereInAsync<Subcategory>("subcategoryId", distinctIds, ct)).ToList();
